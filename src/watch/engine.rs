...
pub struct WatchEngine<'a> {
    src: &'a Path,
    dst: &'a Path,
    ignore: &'a IgnoreMatcher,
}

impl<'a> WatchEngine<'a> {
    ...
    pub fn watch_loop(&self) -> Result<()> {
        ...
        watcher
            .watch(self.src, RecursiveMode::Recursive)
            .with_context(|| format!("Failed to watch source directory: {}", self.src.display()))?;

        println!("watch mode active");
        println!("  src: {}", self.src.display());
        println!("  dst: {}", self.dst.display());
                ...
                Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                        ...
                        && !pending.is_empty()
                    {
                        for path in pending.drain() {
                            ...
                            let Some(dst_path) =
                                crate::sync::map_src_to_dst(self.src, self.dst, &path)
                            else {
                                continue;
                            };

                            if path.exists() {
                                self.handle_present(&path, &dst_path)?;
                            } else {
                                self.handle_removal(&dst_path)?;
                            }
                        }
                        last_event_at = None;
                    }
    ...
    fn handle_present(&self, src: &Path, dst: &Path) -> Result<()> {
        if src.is_dir() {
    ...
    fn handle_removal(&self, dst: &Path) -> Result<()> {
        if !dst.exists() {
            return Ok(());
        }
...
mod tests {
    ...
    fn engine<'a>(root: &'a Path, ignore: &'a IgnoreMatcher) -> WatchEngine<'a> {
        WatchEngine::new(root, root, ignore)
    }
